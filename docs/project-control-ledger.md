...
Timestamp: `2026-05-31 12:45:46 +08:00`.

- Main baseline branch: `main`.
- Latest merged checkpoint covered by this ledger update: `1873ae7da3f5ad27c0ec600ea989fe652627da55 Record attachment storage verification merge`.
- Latest product/test code commit on `main`: `be707b0 Verify asset attachment storage puts`.
- Latest main record commit before this ledger refresh: `1873ae7 Record attachment storage verification merge`.
- Worktree at last check: clean before creating `codex/m3-readiness-ledger-storage-verification-refresh` from current `main`.
...
- `1873ae7da3f5ad27c0ec600ea989fe652627da55 Record attachment storage verification merge`
  ...
  - Attachment route errors now pass through a storage-error allowlist mapper, with stable fallback behavior for unknown provider, S3, and raw storage errors instead of returning storage internals.
  ...
  - This still is not a formal backend-ready sign-off; historical object backfill/schema tightening, production object-store integration plus opt-in S3/MinIO integration tests, orphan object audit/cleanup, browser/manual failure-mode coverage, and broader auth/session/account lifecycle remain open.
- `b4297a70cf4982db4ca5b19dc09316d121d8f2fe Record attachment storage metadata read path merge`
  ...
  - Keeps storage metadata out of the attachment domain object and API response.
  - Attachment downloads now read bytes through the persisted storage key when present and preserve the legacy derived-key fallback.
  - Storage reads verify persisted checksum and byte size integrity before returning attachment bytes.
  - Local `db:smoke` was not run for this documentation refresh.
...
- `4bc6ca6 Add auth scope admin UI API helper`
  ...
  - Sends requests with `credentials: "same-origin"` so signed cookie actor resolution remains server-owned.
  - Keeps the slice scoped: no `m1-dashboard.tsx`, UI runtime, route, schema, migration, smoke, or workflow changes.
  - Error handling throws stable route error codes and falls back to `auth_scope_admin_request_failed` for non-JSON failures.
  - Child review 64 found no P0/P1/P2/P3. Real local `db:smoke` was not run.
  - Validation before merge:
    - `npm.cmd run test -w apps/web -- app/ui/auth-scope-admin-api.test.ts`: passed, 1 file / 4 tests.
    - `npm.cmd run typecheck -w apps/web`: passed.
    - `git diff --check main..codex/m3-auth-scope-admin-ui-api`: passed.
...
- `653da0f Overlay DB auth scope in workspace snapshot`
  ...
  - Keeps `currentUserId` / local session pointer in the local workspace while validating the actor against DB-backed users and scope arrays.
  - Updates asset-lock DB repository composition, workspace snapshot overlay, workspace actor behavior, service tests, and Postgres smoke seed/cleanup for auth/scope rows.
  - Keeps the slice scoped: no DB session/cookie/auth login persistence, no workspace-session storage migration, no auth/scope write APIs, no delivery package mutation DB path, no UI/browser QA, and no object storage or attachment byte changes.
  - Child review 29 found no P0/P1/P2/P3 and approved merge. Real `db:smoke` was not run because no `TEST_DATABASE_URL` is available in the current environment.
  - Validation after merge:
    - `npm.cmd run test -w apps/web -- app/api/auth-scope/db-repository.test.ts app/api/asset-lock-records/db-repository.test.ts app/api/asset-lock-records/service.test.ts app/api/asset-lock-attachments/service.test.ts app/api/asset-decision-timeline/service.test.ts`: passed, 5 files / 100 tests.
    - `npm.cmd run db:check -w apps/web`: passed.
    - `npm.cmd run typecheck -w apps/web`: passed.
    - `git diff --check`: passed.
- `a4c8515 test: cover auth scope db read mapping`
  - Adds focused tests for the auth/scope DB read helper that were identified as a P3 coverage gap in review 24.
  - Covers empty rows returning empty arrays, preserving multiple roles for the same user/project, full PermissionKey preservation, empty and custom member permission arrays, and episode/assignment count, boolean, and responsibility mapping.
  - Keeps the slice scoped to `apps/web/app/api/auth-scope/db-repository.test.ts`; no implementation, schema/migration, asset-lock repository, runtime overlay, or UI changes.
  - Child review 28 found no P0/P1/P2/P3 and approved merge.
  - Validation after merge was included in the combined auth/scope overlay validation above.
...
- `22d5968 Add asset lock DB repository skeleton`
  ...
  - Fast-forward merged `codex/m3-record-db-repository-skeleton` into `main` after R review found no P0/P1/P2.
  - R review P3: DB mode unsupported non-create mutations return the capability error before project permission checks. Routes still require a workspace actor first, so this is not an unauthenticated write risk; revisit if service reuse needs permission-preserving unsupported responses.
  - No migration, migration regeneration, real DB migrate, DB startup, script source binding migration, attachment migration, timeline projection change, UI change, or browser QA was included.
  - Validation before commit:
    - `npm.cmd run typecheck -w apps/web`: passed.
  ...
  - Validation before merge:
    - `git diff --check main..HEAD`: passed.
    - `npm.cmd run test -w apps/web -- app/api/asset-lock-records/repository.test.ts app/api/asset-lock-records/db-repository.test.ts app/api/asset-lock-records/service.test.ts app/api/asset-lock-records/route.test.ts`: passed, 4 files / 45 tests.
    - `npm.cmd run typecheck -w apps/web`: passed.
...
- `086cbcf Cover missing source binding route contract`
  ...
  - Fast-forward merged `codex/source-binding-missing-route-contract` into `main` after targeted test/typecheck/diff verification.
  - No production logic, UI, CSS, browser QA, remote push, or `codex/timeline-mobile-crop-hardening` changes were included.
...
- `e4c354d Cover attachment unauthenticated route contract`
  ...
  - Fast-forward merged `codex/api-contract-minimal-gap` into `main` after targeted test/typecheck/diff verification.
  - No production logic, UI, CSS, browser QA, remote push, or `codex/timeline-mobile-crop-hardening` changes were included.
...
- `c5bd0a6 Cover final lock batch blocker copy`
  - Added a focused helper test for the "selected asset ready but same package/batch still blocked" final-lock copy.
  - No demo seed, backend/API, source-binding mutation, timeline, or CSS changes were included.
- `eda7425 Cover attachment delete list refresh contract`
  - Added a route-level smoke/contract test proving that deleting one of two uploaded asset attachments reduces the active attachment list from two to one.
  - No workbench UI, CSS, or backend behavior changes were included.
- `9b488e7 Record attachment delete handoff state`
  - Recorded the attachment delete contract state, pending polish branch, staged validation notes, and fresh-build timeline QA gap.
...
- For `codex/source-binding-missing-route-contract` after merge into `main`:
...
- For `codex/source-binding-route-permission-contract` after merge into `main`:
  ...
  - `npm.cmd run typecheck -w apps/web`: passed.
  - `git diff --check`: passed, with only Git's LF-to-CRLF warning for `apps/web/app/api/asset-lock-records/route.test.ts`.
- For `codex/api-contract-minimal-gap` after merge into `main`:
  - `npm.cmd run test -w apps/web -- asset-lock-attachments`: passed, 2 files / 31 tests.
  - `npm.cmd run typecheck -w apps/web`: passed.
...
- For asset lock source-binding badges:
  ...
  - `npm.cmd run typecheck -w apps/web`: passed.
  - `git diff --check`: passed.
  - Browser light QA on `main@d293fe8`: passed at 1366, 390, 760, and 1440 widths for list/detail badge layout and locked source-binding panel readability.
  - Browser QA gap: current demo data did not reproduce the specific "selected record ready but same package/batch still blocked" final-lock disabled copy.
- For final-lock copy coverage:
  - `npm.cmd run test -w apps/web -- asset-lock-workbench`: passed, 1 file / 13 tests.
  - `npm.cmd run typecheck -w apps/web`: passed.
  - `git diff --check`: passed.
- For attachment delete list refresh contract:
  - `npm.cmd run test -w apps/web -- asset-lock-attachments asset-lock-attachment-api`: passed, 3 files / 37 tests.
  - `npm.cmd run typecheck -w apps/web`: passed.
  - `git diff --check`: passed.
- For source-binding error copy polish after rebase on `main@9b488e7`:
...
- Active branch: `main`.
- Current local `main` HEAD: `6eed4d4 Add asset attachment row actions`.
- Main includes:
  - `362d900 Cover locked source binding access`.
  - `f145a0f Add asset attachment download and delete APIs`.
  - `a316c77 Record xiezuogongju-04 handoff`.
  - `6eed4d4 Add asset attachment row actions`.
...
Current attachment download/delete status:
...
- UI helper exposes download/delete wrappers.
- Attachment UI is wired in `asset-lock-workbench.tsx`: each attachment row has download/delete actions.
- Download uses a Blob/object URL helper and revokes the object URL in `finally`.
- Delete is disabled for locked records and refreshes the selected record's active attachment list after success.
- Download/delete failures map to Chinese messages; browser failure-state reproduction was blocked, but code review and tests confirmed `Failed to fetch` is not surfaced raw.
- `asset-decision-timeline.tsx` was not modified by the attachment UI slice.
...
- 22: first review was on the wrong baseline and invalid; second review found P1s (locked deletion and list auth), both fixed by 21. Final re-review reported no P0/P1/P2 and allowed UI wiring.
- 23: added locked source-binding helper test; committed as `362d900`.
- 24: designed attachment UI wiring.
- 25: implemented attachment row download/delete actions and fixed network-failure Chinese error fallback.
- 26: code review reported no P0/P1/P2; P2 `next-env.d.ts` generated churn was removed before commit.
- 27: browser review verified locked attachment download/delete-disabled and layout; network failure reproduction was blocked by browser security, so source/tests were used as the final gate.
...
- `npm.cmd run test -w apps/web -- asset-lock-attachments asset-lock-records asset-decision-timeline asset-lock-attachment-api asset-source-binding-data`: passed, 12 files / 124 tests.
- `npm.cmd run test -w packages/domain -- store`: passed, 1 file / 31 tests.
- `npm.cmd run build -w apps/web`: passed.
- `npm.cmd run test -w apps/web`: passed, 23 files / 196 tests.
- `npm.cmd run verify`: passed on `2026-05-27`:
  - web: 23 files / 196 tests.
  - domain: 6 files / 63 tests.
  - Next production build passed.
...
1. Continue from `main@6eed4d4`.
2. Do not push unless the user explicitly asks.
3. Recommended next product slice: asset timeline explicit-source display enhancement, keeping `/api/asset-decision-timeline` GET-only/read-only.
4. Alternative small slice: attachment UI/API smoke automation for non-locked delete success using a controlled API fixture, because in-app browser file upload is limited.
