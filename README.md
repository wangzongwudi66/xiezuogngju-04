...
- 集工作台：展示当前生效剧本、修订记录、diff 摘要入口和 docx 导出占位。
- 创作者视图：不显示交稿中心，只能看到自己负责集的当前剧本状态和关键变更通知。

当前仍是 mock 原型：页面里用预置交稿包演示流程，不做真实 Word 文件上传。Word 自动切段工具已经在领域层实现，后续要接入上传页和异步任务。
...
- 登录/注册是 mock，刷新后恢复 seed 数据。
- 权限是前端原型和领域层函数校验，尚未接真实鉴权服务。
- M2 页面不上传真实 Word 文件；真实上传、异步切段、确认页接入文件解析是下一步。
- docx 导出工具已实现，页面按钮目前是占位，还没有触发下载。
- 不做 PDF。
